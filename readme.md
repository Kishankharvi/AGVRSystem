...
Project: AGVRSystem (Audio Guided VR Rehabilitation System)  
...
6	Provide audio-first accessibility	 TTS with offline clip fallback
...
Main Flow:
Patient sees Main Menu in VR
Patient uses VR keyboard to enter their Patient ID
Patient presses "Start Session" button
System transitions to Calibration scene
Patient holds hands visible in headset camera view
System displays tracked hand skeleton overlay
When tracking confirmed stable, patient presses "Ready"
System transitions to RehabSession scene
Audio narration welcomes patient and describes exercise sequence
Session begins with Exercise 1 (Grip Hold)
...
Main Flow:
HUD displays exercise name, rep target, and instructions
Audio guide narrates the exercise technique
Patient performs gesture (e.g., grips virtual ball)
ExerciseObjectController shows progress bar on object
Hold timer fills on HUD bottom bar
Rep registered; audio plays encouragement
After reaching target reps, exercise auto-advances
HUD shows brief "Exercise Complete! Accuracy: 92%" message
Audio narrates exercise result
Next exercise begins after 2-second pause
Alternate Flow (Tracking Lost):
Hand disappears from camera view
HUD shows "Tracking Lost" badge in red
Hold timer pauses (grace period active)
Audio plays "Please keep your hands visible"
When tracking resumes, exercise continues from paused state
---
UC-03: Session Completes and Data Uploads
Actor: System  
Precondition: All 5 exercises completed  
Main Flow:
ExerciseCoordinator finalizes session
SessionSummaryUI appears with per-exercise breakdown
ReportBoard displays in VR space with results
APIManager sends POST to `http://localhost:8000/api/session`
Server returns 200 OK
Session marked complete
Alternate Flow (Network Unavailable):
APIManager POST fails (timeout / connection refused)
SessionManager saves JSON to device storage
SummaryUI still shows results correctly
On next app launch, RetryOfflineSessions() re-attempts upload
---
UC-04: Therapist Reviews Session Data (External System)
Actor: Clinical Therapist  
Precondition: Session data uploaded to server  
Main Flow:
Therapist opens web dashboard (external system)
Therapist searches by Patient ID
Server returns historical session list
Therapist selects session to view details
Per-exercise accuracy, grip strength, duration displayed
Therapist compares trends across sessions
Therapist adjusts prescribed exercise protocol
---
UC-05: Adaptive Difficulty Adjusts During Session
Actor: System  
Precondition: Patient in exercise, ≥3 reps completed  
Main Flow:
Patient completes rep with accuracy > 85%
System logs accuracy in rolling 3-rep window
All 3 recent reps > 85%
DifficultyMultiplier increases by 0.1
Next GripHold: holdDuration increases from 3.0s to 3.3s
Patient must sustain grip longer for credit
(If accuracy falls < 50%): multiplier decreases, making exercises easier
---
UC-06: Patient Performs Finger Tapping Exercise
Actor: Patient  
Precondition: FingerTappingExercise is the current exercise  
Main Flow:
HUD shows "Tap your index finger to thumb — 20 taps"
Audio narrates tapping technique
Patient repeatedly pinches index finger to thumb
Each pinch strength > 85%: tap registered, counter increments
0.3s cooldown prevents double-counting
Cooldown sound (beep) confirms each tap
Rep counter on HUD increments each tap
At 20 taps: exercise completes, audio celebrates
---
